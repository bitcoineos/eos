...
BOOST_AUTO_TEST_CASE(link_auths) { try {
   ...
   chain.link_authority("alice", "BE", "spending",  "reqauth");
   ...
   BOOST_CHECK_THROW( chain.link_authority("alice", "BE", "spending",  "reqauth"), action_validate_exception);

   // Unlink alice with eosio reqauth
   chain.unlink_authority("alice", "BE", "reqauth");
   ...
   chain.link_authority("alice", "BE", "scud");
...
BOOST_AUTO_TEST_CASE(link_then_update_auth) { try {
   ...
   chain.link_authority("alice", "BE", "first",  "reqauth");
...
try {
   ...
   keys = joe_owner_authority.auth.get_keys();
   BOOST_TEST(keys.size() == 1);
   BOOST_TEST(string(keys[0].key) == chain.get_public_key("joe", "owner").to_addr());
   BOOST_TEST(keys[0].weight == 1);
   ...
   keys = joe_active_authority.auth.get_keys();
   BOOST_TEST(keys.size() == 1);
   BOOST_TEST(string(keys[0].key) == chain.get_public_key("joe", "active").to_addr());
   BOOST_TEST(keys[0].weight == 1);
   ...
   chain.create_account("BE.test1");

   // Creating account with eosio. prefix with non-privileged account, should fail
   BOOST_CHECK_EXCEPTION(chain.create_account("BE.test2", "joe"), action_validate_exception,
                         fc_exception_message_is("only privileged accounts can have names that start with 'BE.'"));
...
BOOST_AUTO_TEST_CASE( any_auth ) { try {
   ...
   chain.link_authority( "alice", "BE", "BE.any", "reqauth" );
   chain.link_authority( "bob", "BE", "BE.any", "reqauth" );
