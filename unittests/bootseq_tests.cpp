...
public:
   ...
   fc::variant get_global_state() {
      vector<char> data = get_row_by_account( N(BE), N(BE), N(global), N(global) );
    ...
    auto buyram( name payer, name receiver, asset ram ) {
       auto r = base_tester::push_action(N(BE), N(buyram), payer, mvo()
                    ("payer", payer)
                    ("receiver", receiver)
                    ("quant", ram)
                    );
    ...
    auto delegate_bandwidth( name from, name receiver, asset net, asset cpu, uint8_t transfer = 1) {
       auto r = base_tester::push_action(N(BE), N(delegatebw), from, mvo()
                    ("from", from )
                    ("receiver", receiver)
    ...
    asset get_balance( const account_name& act ) {
         return get_currency_balance(N(BE.token), symbol(CORE_SYMBOL), act);
    }
...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        create_accounts({N(BE.msig), N(BE.token), N(BE.ram), N(BE.ramfee), N(BE.stake), N(BE.vpay), N(BE.bpay), N(BE.saving) });

        // Set code for the following accounts:
        //  - eosio (code: eosio.bios) (already set by tester constructor)
        //  - eosio.msig (code: eosio.msig)
        //  - eosio.token (code: eosio.token)
        set_code_abi(N(BE.msig), eosio_msig_wast, eosio_msig_abi);//, &eosio_active_pk);
        set_code_abi(N(BE.token), eosio_token_wast, eosio_token_abi); //, &eosio_active_pk);

        // Set privileged for eosio.msig and eosio.token
        set_privileged(N(BE.msig));
        set_privileged(N(BE.token));

        // Verify eosio.msig and eosio.token is privileged
        const auto& eosio_msig_acc = get<account_object, by_name>(N(BE.msig));
        BOOST_TEST(eosio_msig_acc.privileged == true);
        const auto& eosio_token_acc = get<account_object, by_name>(N(BE.token));
        ...
        create_currency(N(BE.token), config::system_account_name, max_supply);
        // Issue the genesis supply of 1 billion SYS tokens to eosio.system
        issue(N(BE.token), config::system_account_name, config::system_account_name, initial_supply);
        ...
        for( const auto& a : test_genesis ) {
           create_account( a.aname, N(BE) );
        }

        // Set eosio.system to eosio
        set_code_abi(N(BE), eosio_system_wast, eosio_system_abi);
        ...
        for( const auto& a : test_genesis ) {
           ...
           auto r = buyram(N(BE), a.aname, asset(ram));
           BOOST_REQUIRE( !r->except_ptr );

           r = delegate_bandwidth(N(BE.stake), a.aname, asset(net), asset(cpu));
           BOOST_REQUIRE( !r->except_ptr );
        }
        ...
        BOOST_TEST(active_schedule.producers.front().producer_name == "BE");
