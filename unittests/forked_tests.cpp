...
BOOST_AUTO_TEST_CASE( forking ) try {
   ...
   auto r2 = c.create_accounts( {N(BE.token)} );
   wdump((fc::json::to_pretty_string(r2)));
   c.set_code( N(BE.token), eosio_token_wast );
   c.set_abi( N(BE.token), eosio_token_abi );
   c.produce_blocks(10);


   auto cr = c.push_action( N(BE.token), N(create), N(BE.token), mutable_variant_object()
              ("issuer",       "BE" )
   ...
   cr = c.push_action( N(BE.token), N(issue), N(BE), mutable_variant_object()
              ("to",       "dan" )
