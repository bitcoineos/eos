...
public:
   ...
   eosio_system_tester(Lambda setup) {
      ...
      create_accounts({ N(BE.token), N(BE.ram), N(BE.ramfee), N(BE.stake),
               N(BE.bpay), N(BE.vpay), N(BE.saving), N(BE.names) });
      ...
      {
         const auto& accnt = control->db().get<account_object,by_name>( N(BE.token) );
         abi_def abi;
   ...
   asset get_balance( const account_name& act ) {
      vector<char> data = get_row_by_account( N(BE.token), act, N(accounts), symbol(CORE_SYMBOL).to_symbol_code().value );
   ...
   fc::variant get_stats( const string& symbolname ) {
      ...
      vector<char> data = get_row_by_account( N(BE.token), symbol_code, N(stat), symbol_code );
   ...
   abi_serializer initialize_multisig() {
      ...
      {
         create_account_with_resources( N(BE.msig), config::system_account_name );
         BOOST_REQUIRE_EQUAL( success(), buyram( "BE", "BE.msig", core_from_string("5000.0000") ) );
         produce_block();
