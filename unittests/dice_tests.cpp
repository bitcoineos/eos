...
BOOST_FIXTURE_TEST_CASE( dice_test, dice_tester ) try {
   ...
   create_accounts( {N(BE.token), N(dice),N(alice),N(bob),N(carol),N(david)}, false);
   ...
   push_action(N(BE.token), N(create), N(BE.token), mvo()
     ("issuer", "BE.token")
     ("maximum_supply", core_from_string("1000000000.0000"))
   );

   push_action(N(BE.token), N(issue), N(BE.token), mvo()
     ("to", "BE")
     ("quantity", core_from_string("1000000000.0000"))
     ("memo", "")
   );

   transfer( N(BE), N(alice), core_from_string("10000.0000"), "", N(BE.token) );
   transfer( N(BE), N(bob),   core_from_string("10000.0000"), "", N(BE.token) );
   transfer( N(BE), N(carol), core_from_string("10000.0000"), "", N(BE.token) );
   ...
   BOOST_REQUIRE_EQUAL(
      get_currency_balance(N(BE.token), symbol(CORE_SYMBOL), N(alice)),
