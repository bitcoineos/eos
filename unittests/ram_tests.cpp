...
BOOST_FIXTURE_TEST_CASE(ram_tests, eosio_system::eosio_system_tester) { try {
   ...
   buyramB(N(BE), N(BE), 70000);
   produce_blocks(10);
   create_account_with_resources(N(testram1),N(BE), init_request_bytes + 40);
   create_account_with_resources(N(testram2),N(BE), init_request_bytes + 1190);
   produce_blocks(10);
   BOOST_REQUIRE_EQUAL( success(), stake( "BE.stake", "testram1", core_from_string("10.0000"), core_from_string("5.0000") ) );
   produce_blocks(10);

   for (auto i = 0; i < 10; ++i) {
      try {
         set_code( N(testram1), test_ram_limit_wast );
         break;
      } catch (const ram_usage_exceeded&) {
         init_request_bytes += increment_contract_bytes;
         buyramB(N(BE), N(testram1), increment_contract_bytes);
         buyramB(N(BE), N(testram2), increment_contract_bytes);
      }
   }
   produce_blocks(10);
   ...
   for (auto i = 0; i < 10; ++i) {
      try {
         set_abi( N(testram1), test_ram_limit_abi );
         break;
      } catch (const ram_usage_exceeded&) {
         init_request_bytes += increment_contract_bytes;
         buyramB(N(BE), N(testram1), increment_contract_bytes);
   ...
   produce_blocks(10);
   set_code( N(testram2), test_ram_limit_wast );
   set_abi( N(testram2), test_ram_limit_abi );
   produce_blocks(10);

   auto total = get_total_stake( N(testram1) );
   const auto init_bytes =  total["ram_bytes"].as_uint64();

   auto rlm = control->get_resource_limits_manager();
   auto initial_ram_usage = rlm.get_account_ram_usage(N(testram1));
   ...
   buyramB(N(BE), N(testram1), more_ram);
   buyramB(N(BE), N(testram2), more_ram);

   TESTER* tester = this;
   // allocate just under the allocated bytes
   tester->push_action( N(testram1), N(setentry), N(testram1), mvo()
                        ("payer", "testram1")
                        ("from", 1)
                        ("to", 10)
                        ("size", 1780 /*1910*/));
   produce_blocks(1);
   auto ram_usage = rlm.get_account_ram_usage(N(testram1));

   total = get_total_stake( N(testram1) );
   ...
   BOOST_REQUIRE_EXCEPTION(
      tester->push_action( N(testram1), N(setentry), N(testram1), mvo()
                           ...
                           ("size", 1790 /*1920*/)),
                           ram_usage_exceeded,
                           fc_exception_message_starts_with("account testram1 has insufficient ram"));
   wlog("ram_tests 2    %%%%%%");
   produce_blocks(1);
   BOOST_REQUIRE_EQUAL(ram_usage, rlm.get_account_ram_usage(N(testram1)));
   ...
   tester->push_action( N(testram1), N(setentry), N(testram1), mvo()
                        ...
                        ("size", 1680/*1810*/));
   produce_blocks(1);
   BOOST_REQUIRE_EQUAL(ram_usage - 1000, rlm.get_account_ram_usage(N(testram1)));
   ...
   tester->push_action( N(testram1), N(rmentry), N(testram1), mvo()
                        ("from", 3)
                        ("to", 3));
   produce_blocks(1);
   ...
   // verify that the new entry is under the allocation bytes limit
   tester->push_action( N(testram1), N(setentry), {N(testram1),N(testram2)}, mvo()
                        ("payer", "testram2")
                        ("from", 12)
                        ("to", 12)
                        ("size", 1720));
   ...
   wlog("ram_tests 18    %%%%%%");
   // verify that the new entry is under the allocation bytes limit, because entry 12 is now charged to testram2
   tester->push_action( N(testram1), N(setentry), N(testram1), mvo()
                        ("payer", "testram1")
                        ("from", 13)
                        ("to", 13)
   ...
   // verify that new entries for testram2 exceed the allocation bytes limit
   BOOST_REQUIRE_EXCEPTION(
      tester->push_action( N(testram1), N(setentry), {N(testram1),N(testram2)}, mvo()
                           ("payer", "testram2")
                           ("from", 12)
                           ...
                           ("size", 1930)),
                           ram_usage_exceeded,
                           fc_exception_message_starts_with("account testram2 has insufficient ram"));
   produce_blocks(1);

   // verify that new entries for testram2 are under the allocation bytes limit
   tester->push_action( N(testram1), N(setentry), {N(testram1),N(testram2)}, mvo()
                        ("payer", "testram2")
                        ("from", 12)
                        ("to", 21)
   ...
   // verify that new entry for testram2 exceed the allocation bytes limit
   BOOST_REQUIRE_EXCEPTION(
      tester->push_action( N(testram1), N(setentry), {N(testram1),N(testram2)}, mvo()
                           ("payer", "testram2")
                           ("from", 22)
   ...
   // verify that new entry for testram2 are under the allocation bytes limit
   tester->push_action( N(testram1), N(setentry), {N(testram1),N(testram2)}, mvo()
                        ("payer", "testram2")
                        ("from", 22)
                        ("to", 22)
