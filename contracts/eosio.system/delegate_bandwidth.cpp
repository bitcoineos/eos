...
namespace eosiosystem {
   ...
   void system_contract::buyram( account_name payer, account_name receiver, asset quant )
   {
      ...
      INLINE_ACTION_SENDER(eosio::token, transfer)( N(BE.token), {payer,N(active)},
         { payer, N(BE.ram), quant_after_fee, std::string("buy ram") } );

      if( fee.amount > 0 ) {
         INLINE_ACTION_SENDER(eosio::token, transfer)( N(BE.token), {payer,N(active)},
                                                       { payer, N(BE.ramfee), fee, std::string("ram fee") } );
      }
   ...
   void system_contract::sellram( account_name account, int64_t bytes ) {
      ...
      INLINE_ACTION_SENDER(eosio::token, transfer)( N(BE.token), {N(BE.ram),N(active)},
                                                       { N(BE.ram), account, asset(tokens_out), std::string("sell ram") } );

      auto fee = ( tokens_out.amount + 199 ) / 200; /// .5% fee (round up)
      // since tokens_out.amount was asserted to be at least 2 earlier, fee.amount < tokens_out.amount
      
      if( fee > 0 ) {
         INLINE_ACTION_SENDER(eosio::token, transfer)( N(BE.token), {account,N(active)},
            { account, N(BE.ramfee), asset(fee), std::string("sell ram fee") } );
      }
   }
                                   ...
                                   const asset stake_net_delta, const asset stake_cpu_delta, bool transfer )
   {
      ...
      if ( N(BE.stake) != source_stake_from ) { //for eosio both transfer and refund make no sense
         ...
         if ( asset(0) < transfer_amount ) {
            INLINE_ACTION_SENDER(eosio::token, transfer)( N(BE.token), {source_stake_from, N(active)},
               { source_stake_from, N(BE.stake), asset(transfer_amount), std::string("stake bandwidth") } );
         }
      }
   ...
   void system_contract::refund( const account_name owner ) {
      ...
      INLINE_ACTION_SENDER(eosio::token, transfer)( N(BE.token), {N(BE.stake),N(active)},
                                                    { N(BE.stake), req->owner, req->net_amount + req->cpu_amount, std::string("unstake") } );
